...
module.exports = {
    ...
    submitEnquiry:(data,callBack)=>{
        pool.query("INSERT INTO enquire(name,phone,email,sector,referral) values (?,?,?,?,?)",
        [data.name,data.phone,data.email,data.sector,data.referral],
        (err,result,field)=>{
            if(err){
                return callBack(err,null);
            }
